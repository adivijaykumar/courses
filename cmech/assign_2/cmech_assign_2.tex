...
\begin{homeworkProblem}
	...
	\bes
	\alpha  g k x^2 \left(\frac{x}{l}\right)^{\alpha }+(\alpha -1) \alpha ^2 k^2 \dot{x}^2 \left(\frac{x}{l}\right)^{2 \alpha }+\alpha ^2 k^2 x \ddot{x} \left(\frac{x}{l}\right)^{2 \alpha }-x^4 \omega ^2+x^3 \ddot{x}=0
	\ees
...
\begin{homeworkProblem}
	...
	\ees
	We choose $ \psi $ and $ \psi^* $ as our generalized coordinates, and $ (t,x) $ as the dependent coordinates. One can write the equations of motion in a compact form as follows,
	
	\bes
	\partial_\mu \qty(\pdv{L}{(\partial_\mu \psi)}) = \pdv{L}{\psi} \qq{and} \partial_\mu \qty(\pdv{L}{(\partial_\mu \psi^*)}) = \pdv{L}{\psi^*}
	\ees
	
	where the index $ \mu $ goes over $ (t,x) $.
	\\
	...
	\ees
	\bes
	\therefore \frac{y}{\sqrt{1+y'^2}} = \alpha \implies y = \alpha \cosh \qty(\frac{x}{\alpha} + \beta)
	\ees	
	...
	\bes
	\dv{\sin^2\theta \phi'}{\theta} = 0 \implies \phi' = \alpha \csc[2](\theta) \implies \phi(\theta) = a \cot\theta + b
	\ees
