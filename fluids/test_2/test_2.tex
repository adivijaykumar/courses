...
\begin{homeworkProblem}[2]
	...
	\begin{align*}
	S_2 (l) &\sim \int_{0}^{\infty} e^{i kl} k^{-n} dk\\
	&\sim \int_{0}^{\infty} e^{i x} (x)^{-n} l^n \dfrac{dx}{l} \impliedby \qq{substituting} x = kl\\
...
\begin{homeworkProblem}[3]
	As the depth and the width for all three streams is the same and constant, we expect that at the source on top of the mountain, the volume flow gets divided equally between the streams.
	\begin{equation*}
	P_{atm} + \dfrac{Q^2}{18 A^2} + gH = P_{atm} + \dfrac{Q^2}{18 A^2} + gH 
	\end{equation*}
\end{homeworkProblem}
