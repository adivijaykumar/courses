...
\textbf{a)} Prove that eigenvalues of skew symmetric matrix are complex\\

Let $A$ be a skew symmetric matrix, ie $A^{\dagger} = -A$. 
$$AX = \lambda X$$
, where $\lambda$ is the eigenvalue. Taking the conjugate transpose of the above equation, one gets
$$A = \lambda X$$
