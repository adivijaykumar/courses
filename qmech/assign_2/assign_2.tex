...
\begin{homeworkProblem}
	...
	Given that,
	\begin{equation*}
	x(\tau) = \sum_n x_n e^{\frac{2\pi i n \tau}{\beta}} = \sum_n (a_n + i b_n) e^{\frac{2\pi i n \tau}{\beta}}
	\end{equation*}
	...
	\begin{align*}
	Z_c(\beta) &= N \int \mathcal{D}x \exp - \frac{m\beta}{2}\sum_{q}x_{q}^* x_q   \qty(\omega_q^2 + \omega^2 ) \qq{where $ N $ is some normalization}\\
	...
	\end{align*}
	One can also calculate the quantity $ Z_{free}(\beta) $ which corresponds to $ \omega = 0 $. Following the steps above, that comes out to be,
	\begin{equation*}
	Z_{free}(\beta) = N \prod_{q=1}^\infty \frac{2\pi}{m \beta \omega_q^2}
	\end{equation*}
	Our final partition function is,
	\begin{align*}
	Z(\beta) = \frac{Z_c(\beta)}{Z_{free}(\beta)} = \sqrt{\frac{2\pi}{m \beta \omega^2}} \cross \prod_{q=1}^\infty \frac{\omega_q^2}{\omega_q^2 + \omega^2}
	\end{align*}
\end{homeworkProblem}
%\pagebreak 	
