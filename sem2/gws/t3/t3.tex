...
\begin{homeworkProblem}
	...
	\textbf{Parts (c) and (d)}\\
	Radial null geodesics will have $ \kappa =0  $ and $ L=0 $. Hence, using (\ref{e8}),
	\begin{align}\label{key}
	\dot{r}^2 &= E^2  \\
	\implies \dv{r}{\lambda} &= - E \qq{for ingoing rays}\\
	\implies \int_{r_0}^{2M} \dd{r}  &= - E \int_{0}^{\lambda_0} \dd{\lambda} \\
	\implies \lambda_0 &= \dfrac{r_0 - 2M}{E} \implies \qq{finite}
	\end{align}
	
	Also, using (\ref{e2}), and assuming null radial orbits, we get,
	\begin{align}\label{key}
	\dfrac{\dot{t}^2}{\dot{r}^2} &= \qty(\dv{t}{r})^2 = \qty(1 - \dfrac{2M}{r})^{-2}\\
	\implies t &= \pm \int \dd{r}  \qty(1 - \dfrac{2M}{r})^{-1} + C \\
	\implies t &= \pm \qty[r - r_0 + 2M \log(\dfrac{r-2M}{r_0 - 2M})] + t_0
	\end{align}
	
	\textcolor{red}{Sketch the rays}
\end{homeworkProblem}
